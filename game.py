...
dist_between_pipes=250
pipe_speed=8
pipe_velocity=lambda:pipe_speed
score=0
...
with mp_face_mesh.FaceMesh(max_num_faces=1,
    ...
    while True:
        if not game_is_running:
               text=pygame.font.SysFont("helvetica",50).render("Game Over! Skill issue",True,(153,0,0))
               tr=text.get_rect()
        ...
        #stage, score text
        text=pygame.font.SysFont("Arial",50).render(f"Stage: {stage}",True,(99,245,255))
        tr=text.get_rect()
        tr.center=(100,50)
        screen.blit(text,tr)
        text=pygame.font.SysFont("Arial",45).render(f"Score: {score}",True,(99,245,255))
        tr=text.get_rect()
        tr.center=(100,100)
        screen.blit(text,tr)
        ...
        if pipeSpawnTimer == 0:
           ...
           gap_y = random.randint(20, int(window_size[1] - 50 - gap))
        ...
        if time.time()-game_clock>=10:
            time_between_pipe_spawn = max(20,int(time_between_pipe_spawn * 0.9) )
            space_between_pipes = max(180, int(space_between_pipes * 0.95))       
            stage+=1
            pipe_speed += 1
            game_clock = time.time()
